...
#include "DucksStandardMessage.h"
#include <cassert>
#include <algorithm>
...
namespace Ducks
{
  ...
  {
    static_assert(DUCKS_STANDARD_MESSAGE_STRUCT_VERSION == 0, "The constructor is not adjusted for the current message version");
    return 5;
  }
  ...
  {
    static_assert(DUCKS_STANDARD_MESSAGE_STRUCT_VERSION == 0, "The method is not adjusted for the current message version");
  ...
  void OwnTeamInfo::write(void*& data, uint32_t timestamp) const
  {
    ...
    state.write(data);
//    if (state.gameState == static_cast<uint16_t>(GameState::PLAYING) && state.setPlay != static_cast<uint16_t>(SetPlay::NONE)) { TODO LEGACY CODE
...
//    writeVal<uint16_t>(data, static_cast<uint16_t>(std::min(63u, 1u + dropInTime) >> 1 << 11 | (secsRemaining & 0x3FF) << 1 | (secondaryTime >> 8 & 1)));
    writeVal<uint16_t>(data, static_cast<uint16_t>(std::min(63u, 1u + 0) >> 1 << 11 | (secsRemaining & 0x3FF) << 1 | (secondaryTime >> 8 & 1))); //TODO LEGACY CODE (0)
    writeVal<uint16_t>(data, static_cast<uint16_t>(secondaryTime));

    uint16_t scorePlusIsPenalized = score;
    for(unsigned i = 0; i < DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS; ++i)
...
//    dropInTime = static_cast<uint16_t>(timeStruct >> 11 << 1);
    ...
    score = scorePenaltyStruct >> DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS;
    unsigned runner = 1 << (DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS - 1);
    for(unsigned i = 0; i < DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS; ++i, runner >>= 1)
      playersArePenalized[i] = (runner & scorePenaltyStruct) != 0;
  }

  /*
   * =============================================
   * ============ DUCKS STANDARD MESSAGE ========
   * =============================================
   */

  DucksStandardMessage::DucksStandardMessage() :
    version(DUCKS_STANDARD_MESSAGE_STRUCT_VERSION),
    member(-1),
    timestamp(0),
    isUpright(true),
    ...
    gameControlData(),
    headYawAngle(0.f),
    currentlyPerfomingRole(PlayingRole::NONE),
    ...
    ntpMessages()
  {
    ...
    const char* init = DUCKS_STANDARD_MESSAGE_STRUCT_HEADER;
    for(unsigned int i = 0; i < sizeof(header); ++i)
      header[i] = init[i];

    roleAssignments[0] = PlayingRole::KEEPER;
    for(int i = 1; i < DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS; ++i)
      roleAssignments[i] = PlayingRole::STRIKER;
  }

  int DucksStandardMessage::sizeOfDucksMessage() const
  {
    static_assert(DUCKS_STANDARD_MESSAGE_STRUCT_VERSION == 0, "This method is not adjusted for the current message version");
    ...
    return sizeof header
           ...
           + 1 // /\, passTarget, \/
           + std::min(int(obstacles.size()), DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_OBSTACLES) * Obstacle::sizeOfObstacle()
           + 2 // member, isUpright, hasGroundContact, isPenalized, kingIsPlayingBall, requestsNTPMessage, \/
           + static_cast<int>(ntpMessages.size()) * 5;
    ;
  }

  bool DucksStandardMessage::read(const void* data)
  {
    static_assert(DUCKS_STANDARD_MESSAGE_STRUCT_VERSION == 0, "This method is not adjusted for the current message version");
    ...
    if(version != DUCKS_STANDARD_MESSAGE_STRUCT_VERSION)
      return false;
    ...
    static_assert(static_cast<unsigned>(PlayingRole::NUM_ROLES) <= 8u, "Following does not work for that many roles. Adjust it!");
    const uint32_t roleContainer = readVal<const uint32_t>(data);

    static_assert(DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS == 10, "Following does not work for that many teammates. Adjust it!");
    unsigned runner = 0x7 << ((DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS - 1) * 3);
    for(unsigned i = 0; i < DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS; ++i, runner >>= 3)
      roleAssignments[i] = static_cast<PlayingRole>((roleContainer & runner) >> ((DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS - i - 1) * 3));

    static_assert(DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_OBSTACLES < 8, "Following does not work for that many obstacles. Adjust it!");
    static_assert(DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS < 16, "Following does not work for that many teammates. Adjust it!");
    ...
    currentlyPerfomingRole = static_cast<PlayingRole>(((numObsCurrPerforRolePassTargContainer & 8) >> 1) | (roleContainer >> (DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS * 3)));
    ...
    assert((reinterpret_cast<const char*>(data) - reinterpret_cast<const char*>(begin)) == sizeOfDucksMessage());
    return true;
  }

#define CUT_TO_PLUS_MINUS(value, positiveRange) std::max(std::min( (value), positiveRange), -positiveRange)
  void DucksStandardMessage::write(void* data) const
  {
    static_assert(DUCKS_STANDARD_MESSAGE_STRUCT_VERSION == 0, "This method is not adjusted for the current message version");
    ...
    for(int i = 0; i < DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS; ++i)
      (roleContainer <<= 3) |= static_cast<uint32_t>(roleAssignments[i]);
    writeVal<uint32_t>(data, roleContainer); //losing 1 bit; taking it next

    static_assert(DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_OBSTACLES < 8, "Following does not work for that many obstacles. Adjust it!");
    static_assert(DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS < 16, "Following does not work for that many teammates. Adjust it!");
    const int numOfObstacles = std::min(DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_OBSTACLES, static_cast<int>(obstacles.size()));
    ...
    if(!ntpMessages.empty())
    {
      ...
      for(unsigned i = 0; i < DUCKS_STANDARD_MESSAGE_MAX_NUM_OF_PLAYERS; ++i, ntpReceivers <<= 1)
