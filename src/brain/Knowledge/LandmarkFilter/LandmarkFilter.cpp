...
{
  {
    ...
    if (lineData_->valid)
    {
      ...
      if (penaltySpotData_->valid)
      {
        landmarkModel_->penaltyAreas =
            findPenaltyAreas(penaltySpotData_->penaltySpot.relativePosition, linesWithMetaData);
               ...
               cycleInfo_->getTimeDiff(ballState_->timeWhenLastSeen) < 0.2f)
      {
      ...
      landmarkModel_->timestamp = imageData_->timestamp;
    }
  }
  // do debug stuff
  sendDebugImage();
...
std::vector<LineInfo> LandmarkFilter::filterLines(const std::vector<LineInfo>& linesWithMetaData)
{
  ...
  {
    const LineInfo& lineInfo = linesWithMetaData[i];
                                 ...
                                 const std::vector<LineInfo>& linesWithMetaData)
{
  ...
  const Vector2f orientationVec = Geometry::getPointToLineVector(
      *(orientationLineWithMetaData->line), relativePenaltySpotPosition);
  ...
  if (debug().isSubscribed(mount_ + "." + imageData_->identification + "_image"))
  {
    ...
    for (auto& centerCircle : landmarkModel_->centerCircles)
    {
      if (centerCircle.hasOrientation)
      {
        ...
        cameraMatrix_->robotToPixel(lineThroughMiddle.p1, pixelCoordsLineP1);
        cameraMatrix_->robotToPixel(lineThroughMiddle.p2, pixelCoordsLineP2);
        ...
        image.line(pixelCoordsLineP1, pixelCoordsLineP2, Color::RED);
      }
      cameraMatrix_->robotToPixel(centerCircle.position, pixelCoordsCenter);
      pixelCoordsCenter = Image422::get444From422Vector(pixelCoordsCenter);
      image.cross(pixelCoordsCenter, 15, Color::BLUE);
    }
    ...
    for (const auto& penaltyArea : landmarkModel_->penaltyAreas)
    {
      if (penaltyArea.hasOrientation)
      {
        ...
        cameraMatrix_->robotToPixel(orientationLine.p1, pixelCoordsLineP1);
        cameraMatrix_->robotToPixel(orientationLine.p2, pixelCoordsLineP2);
        pixelCoordsLineP1 = Image422::get444From422Vector(pixelCoordsLineP1);
        pixelCoordsLineP2 = Image422::get444From422Vector(pixelCoordsLineP2);
        image.line(pixelCoordsLineP1, pixelCoordsLineP2, Color::RED);
      }
      cameraMatrix_->robotToPixel(penaltyArea.position, pixelCoordsCenter);
      pixelCoordsCenter = Image422::get444From422Vector(pixelCoordsCenter);
      image.cross(pixelCoordsCenter, 15, Color::BLUE);
    }
    ...
    for (auto& intersection : landmarkModel_->intersections)
    {
      ...
      cameraMatrix_->robotToPixel(line1.p1, pixelCoordsLineP1);
      cameraMatrix_->robotToPixel(line1.p2, pixelCoordsLineP2);
      pixelCoordsLineP1 = Image422::get444From422Vector(pixelCoordsLineP1);
      pixelCoordsLineP2 = Image422::get444From422Vector(pixelCoordsLineP2);
      image.line(pixelCoordsLineP1, pixelCoordsLineP2, color);

      cameraMatrix_->robotToPixel(line2.p1, pixelCoordsLineP1);
      cameraMatrix_->robotToPixel(line2.p2, pixelCoordsLineP2);
      pixelCoordsLineP1 = Image422::get444From422Vector(pixelCoordsLineP1);
      pixelCoordsLineP2 = Image422::get444From422Vector(pixelCoordsLineP2);
      image.line(pixelCoordsLineP1, pixelCoordsLineP2, color);
