...
void CollisionDetector::updateOutput()
{
  ...
  { // if half of the past cylces are positive
    ...
    timeOfLastDetection_[SIDE_LEFT] = TimePoint::getCurrentTime();
  }
  if (indicator[SIDE_RIGHT] > lastStates_.size() / 2)
  { // if half of the past cylces are positive
    collisionDetectorData_->collisionRight = true;
    timeOfLastDetection_[SIDE_RIGHT] = TimePoint::getCurrentTime();
  }
  // duel
  ...
  collisionDetectorData_->collisionLeftRigid =
      cycleInfo_->getTimeDiff(timeOfLastDetection_[SIDE_LEFT]) < timeHoldState_();
  // right
  collisionDetectorData_->collisionRightRigid =
      cycleInfo_->getTimeDiff(timeOfLastDetection_[SIDE_RIGHT]) < timeHoldState_();
  // dueling
