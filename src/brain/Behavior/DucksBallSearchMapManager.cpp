#include "DucksBallSearchMapManager.hpp"

#include "Modules/NaoProvider.h"
#include "Tools/Chronometer.hpp"

DucksBallSearchMapManager::DucksBallSearchMapManager(const ModuleManagerInterface& manager)
  : Module(manager)
...
void DucksBallSearchMapManager::cycle()
{
...
void DucksBallSearchMapManager::updateMap()
{
      ...
      abs(absoluteOwnBall.y()) > fieldDimensions_->fieldWidth / 2.f + minBallOutDistance_())
  {
    ...
    cell = &ballSearchMap_->cellFromPosition(
        Vector2f(std::max(ballProjection.x() - 1.f,
                          -fieldDimensions_->fieldThrowInLineLength / 2.f),
                 ballProjection.y()));
    cell->probability = std::max(minProbOnUpvote_(), cell->probability * 1.1f);

    cell = &ballSearchMap_->cellFromPosition(Vector2f(
        std::min(ballProjection.x() + 1.f, fieldDimensions_->fieldThrowInLineLength / 2.f),
        ballProjection.y()));
    cell->probability = std::max(minProbOnUpvote_(), cell->probability * 1.1f);
  }
  ...
  if (gameControllerState_->setPlay == SetPlay::GOAL_FREE_KICK)
  {
    ...
    cell->probability = std::max(minProbOnUpvote_(),
                                 cell->probability * ((confidentBallMultiplier_() - 1) / 2.f + 1));
  }
...
void DucksBallSearchMapManager::integrateRobotKnowledge(const TeamPlayer& player)
{
...
void DucksBallSearchMapManager::resetMap()
{
...
void DucksBallSearchMapManager::resetMapForReady()
{
  resetMap();
...
void DucksBallSearchMapManager::distributeProbability(const Vector2f& p1, const Vector2f& p2,
...
void DucksBallSearchMapManager::rebuildProbabilityMap()
{
