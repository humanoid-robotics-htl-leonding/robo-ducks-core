...
#include "DucksPlayingRoleProvider.hpp"


using PRP = DucksPlayingRoleProvider;

PRP::DucksPlayingRoleProvider(const ModuleManagerInterface &manager)
	: Module(manager),
...
void PRP::cycle()
{
	...
	auto active = std::vector<bool>(maxNumber, false);
	for (const auto &teamPlayer : teamPlayers_->players) {
		active[teamPlayer.playerNumber-1] = !teamPlayer.penalized;
	}
	active[playerConfiguration_->playerNumber-1] = gameControllerState_->penalty == Penalty::NONE;
	...
	for(unsigned int i = 0; i < maxNumber; i++){
		if(active[i]){
			updateRole(i+1, roles[index++]);
			if(roles[index] == PlayingRole::NONE){
				break;
			}
		}
	}
...
bool PRP::playerOneIsFarAway()
{
	...
	playerOneWasFarAway_ = Hysteresis<float>::greaterThan(playerOneToOwnGoal.norm(),
														  playerOneDistanceThreshold_(),
														  hysteresis,
														  playerOneWasFarAway_);
...
void PRP::assignRemainingPlayerRoles()
{
	...
	for (auto &player : remainingPlayers) {
		switch (lastRoleOf(player.playerNumber)) {
			case PlayingRole::DEFENDER:player.x -= 0.2f;
				break;
			case PlayingRole::SUPPORT_STRIKER:player.x += 0.2f;
				break;
			case PlayingRole::BISHOP:player.x += 0.3f;
				break;
			default:break;
		}
	}
...
PlayingRole DucksPlayingRoleProvider::toRole(const std::string &configRole) const
{
