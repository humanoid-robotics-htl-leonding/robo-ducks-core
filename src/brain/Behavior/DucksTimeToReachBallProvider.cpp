...
#include "DucksTimeToReachBallProvider.hpp"


DucksTimeToReachBallProvider::DucksTimeToReachBallProvider(const ModuleManagerInterface& manager)
  : Module(manager)
...
void DucksTimeToReachBallProvider::cycle()
{
  ...
  if ((gameControllerState_->gameState != GameState::PLAYING &&
      ...
      teamBallModel_->ballType == TeamBallModel::BallType::NONE || !ducksStrikerAction_->valid)
  {
    return;
  }
  ...
  const Vector2f ballToTarget = ducksStrikerAction_->target - teamBallModel_->position;
